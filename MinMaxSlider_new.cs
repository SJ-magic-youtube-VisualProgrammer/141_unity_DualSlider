...
namespace Min_Max_Slider
{
	...
	public class MinMaxSlider : Selectable, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerDownHandler
	{
		...
		protected override void Start()
		{
			...
			update_Slider_HandleText_MidGraphic_OnValueChanged(true);
		}
		
		public float get_minValue() { return minValue; }
		public float get_maxValue() { return maxValue; }
		public float get_minLimit() { return minLimit; }
		public float get_maxLimit() { return maxLimit; }
		
		public void Set_minValue(float Value, bool notify = true) { SetValues(Value, maxValue, notify); }
		public void Set_maxValue(float Value, bool notify = true) { SetValues(minValue, Value, notify); }
		public void Set_minLimit(float Value, bool notify = true) { SetLimits(Value, maxLimit, true, notify); }
		public void Set_maxLimit(float Value, bool notify = true) { SetLimits(minLimit, Value, true, notify); }
		
		public void SetLimits(float _minLimit, float _maxLimit, bool b_update, bool notify = true)
		{
			minLimit = wholeNumbers ? Mathf.RoundToInt(_minLimit) : _minLimit;
			maxLimit = wholeNumbers ? Mathf.RoundToInt(_maxLimit) : _maxLimit;
			
			if(minValue < minLimit) minValue = minLimit;
			if(maxLimit < maxValue) maxValue = maxLimit;
			
			if(b_update) update_Slider_HandleText_MidGraphic_OnValueChanged(notify);
		}

		public void SetValues(MinMaxValues values, bool notify = true)
		{
			SetValues(values.minValue, values.maxValue, notify);
		}

		public void SetValues(float _minValue, float _maxValue, bool notify = true)
		{
			if(_minValue < minLimit) _minValue = minLimit;
			if(maxLimit < _maxValue) _maxValue = maxLimit;
			
			minValue = wholeNumbers ? Mathf.RoundToInt(_minValue) : _minValue;
			maxValue = wholeNumbers ? Mathf.RoundToInt(_maxValue) : _maxValue;
			
			update_Slider_HandleText_MidGraphic_OnValueChanged(notify);
		}

		public void SetValues(float _minValue, float _maxValue, float _minLimit, float _maxLimit, bool notify = true)
		{
			SetLimits(_minLimit, _maxLimit, false, false);
			SetValues(_minValue, _maxValue, true);
		}
		
		void update_Slider_HandleText_MidGraphic_OnValueChanged(bool notify = true){
			RefreshSliders();
			UpdateText();
			UpdateMiddleGraphic();
		...
		public void OnPointerDown(PointerEventData eventData)
		{
			...
			if ( dragStartValue < dragStartMinValue01 + (minHandle.rect.width/2 / sliderBounds.rect.width) )
			{
			...
			else if ( dragStartValue > dragStartMaxValue01 - (maxHandle.rect.width/2 / sliderBounds.rect.width) )
			{
